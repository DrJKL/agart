...
//		setRandomAttributes();
	...
	public Organism(Environment env, String str, Strain xStrain, int r, int c,
			int gen, int mut, int mutX, int met, int rpr, int rprX, int cap,
			int rX, int gX, int bX, int nX, int eX, int sX, int wX) {
		orgName = str;
		strain = xStrain;
		causeOfDeath = codDef;
		envr = env;
	...
	public void check() {
		...
		if (envr.getActiveStrainSize(strain) > popCap
	...
	private boolean onEdge() {
		return row == 0 || col == 0 || col == envr.width - 1
				|| row == envr.height - 1;
	}
	...
	public Organism repr() {
		...
		while (r == row || c == col || r < 0 || c < 0
				|| r >= envr.image.getHeight() || c >= envr.image.getWidth()) {
			r = randomInt(row - 3, row + 3);
			c = randomInt(col - 3, col + 3);
		}
		int gen = generation + 1;
		String str = orgName + getChildrenSpawned();
		...
		if (Math.random() * 100 > mutation)
			mut = Math
					.abs(randomInt(mutation - mutationX, mutation + mutationX));
		else
			mut = mutation;

		if (Math.random() * 100 > mutation)
			mutX = Math.abs(randomInt(mutationX - mutationX, mutationX
					+ mutationX));
		else
			mutX = mutationX;
		...
		if (Math.random() * 100 > mutation)
			met = Math
					.abs(randomInt(moveCost - mutationX, moveCost + mutationX));
		else
			met = moveCost;

		if (Math.random() * 100 > mutation)
			rpr = Math
					.abs(randomInt(reprCost - mutationX, reprCost + mutationX));
		else
			rpr = reprCost;
		
		if (Math.random() * 100 > mutation)
			rprX = Math
					.abs(randomInt(reprX - mutationX, reprX + mutationX));
		else
			rprX = reprX;

		if (Math.random() * 100 > mutation)
			cap = Math.abs(randomInt(energyCap - mutationX, energyCap
					+ mutationX));
		else
			cap = energyCap;
		...
		Organism spawn = new Organism(envr, str, strain, r, c, gen, mut, mutX,
				met, rpr, rprX, cap, rX, gX, bX, nX, eX, sX, wX);
	...
	public void move(String dir) {
		...
		int direction = -1;
		char dirChar = dir.toLowerCase().charAt(0);
	...
	/*
	 * nX = 3 eX = 5 sX = 7 wX = 9 totX = 24
	 * 
	 * 01, 02, 03 = north 04, 05, 06, 07, 08 = east 09, 10, 11, 12, 13, 14, 15 =
	 * south 16, 17, 18, 19, 20, 21, 22, 23, 24 = west
	 * 
	 * north ( >0 , <= nX ) east ( >nX , <= nX+eX ) south ( >nX+eX , <=
	 * nX+eX+sX) west ( >nX+eX+sX, <= totX )
	 */
	public void move(int nX, int eX, int sX, int wX) {
		int totX = nX + eX + sX + wX;
		if (totX < 1)
			return;
		int rand = randomInt(1, totX);
		if (rand > 0 && rand <= nX)
			move(0);
	...
	private boolean hasSpace() {
		for (int i = getRow() - 3; i <= getRow() + 3; i++) {
			for (int j = getCol() - 3; i <= getCol() + 3; j++) {
				if (i < 0 || j < 0 || i >= envr.image.getHeight()
						|| j >= envr.image.getWidth()) {
					continue;
	...
	public int distanceFrom(int x, int y) {
		...
		int change = (int) Math.sqrt((deltX * deltX) + (deltY * deltY));
	...
	public int viewMaxGreen() {
		...
		int maxGreen = 0;
		for (int i = 0; i < view.size(); i++) {
			int currGreen = view.get(i).getGreen();
	...
	public int viewMaxBlue() {
		...
		int maxBlue = 0;
		for (int i = 0; i < view.size(); i++) {
			int currBlue = view.get(i).getBlue();
			if (currBlue >= maxBlue) {
				maxIdx = i;
				maxBlue = currBlue;
			}
	...
	public int viewSecondRed() {
		setView();
		ArrayList<Color> newView = (ArrayList<Color>) view.clone();
		newView.set(this.viewMaxRed(), null);
		int maxIdx = 0;
		int maxRed = 0;
		for (int i = 0; i < newView.size(); i++) {
			if (newView.get(i) != null) {
				int currRed = newView.get(i).getRed();
				if (currRed >= maxRed) {
					maxIdx = i;
					maxRed = currRed;
				}
	...
	public int viewSecondGreen() {
		...
		for (int i = 0; i < newView.size(); i++) {
			if (newView.get(i) != null) {
				int currGreen = newView.get(i).getGreen();
				if (currGreen >= maxGreen) {
					maxIdx = i;
					maxGreen = currGreen;
				}
	...
	public int viewSecondBlue() {
		...
		for (int i = 0; i < newView.size(); i++) {
			if (newView.get(i) != null) {
				int currBlue = newView.get(i).getBlue();
				if (currBlue >= maxBlue) {
					maxIdx = i;
					maxBlue = currBlue;
				}
	...
	public int viewMaxAll() {
		...
		for (int i = view.size() - 1; i >= 0; i--) {
			if (view.get(i) != null) {
				Color check = view.get(i);
				int currAll = check.getBlue() + check.getRed()
						+ check.getGreen();
				if (currAll >= maxAll) {
					maxIdx = i;
					maxAll = currAll;
	...
	/*
	 * rX, gX, bX = totalChance
	 * 
	 * 3, 4, 5 = 12
	 * 
	 * 1 2 3 = Red 1 >= r < Green
	 * 
	 * 4 5 6 7 = Green Green <= r <= Red+Green
	 * 
	 * 8 9 10 11 12 = Blue Red+Green < r <= totalChance
	 */
	public void acquireRand(int rX, int gX, int bX) {
		int totX = rX + gX + bX;
		if (envr.getRed(col, row) + envr.getBlue(col, row)
				+ envr.getGreen(col, row) < lowestRGBPer)
			return;
		if (totX < 1)
			return;
		int rand = randomInt(1, totX);
	...
	public void acquireRed() {
		if (envr.getRed(col, row) > lowestRGBPer) {
			int orig = envr.getRed(col, row);
			int take = randomInt(1, 20);
			red += take;
	...
	public void acquireGreen() {
		if (envr.getGreen(col, row) > lowestRGBPer) {
			int orig = envr.getGreen(col, row);
			int take = randomInt(1, 20);
			green += take;
			resourcesGathered += take;
			energy += take;
	...
	public void setStrain(Strain strainSet) {
		strain = strainSet;
		String newName = strainSet.getStrainName()
