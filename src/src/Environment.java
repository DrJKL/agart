...
public class Environment {
	...
	int youngestIn = 0;
	int lastStrain = 0;
	String[] strainNameMods = { "A", "B", "C", "D", "E", "F", "G", "H", "I",
...
//			throw new RuntimeException("too many critters");
		r = checkRow(r);
		c = checkCol(c);
		Organism next = new Organism(this, new defaultStrain(), r, c);
	...
	public Organism addOneAt(Strain str, int xMin, int yMin, int xMax, int yMax) {
		...
		if (yMax >= height)
			yMax = height - 1;
		int orgX = randomInt(xMin, xMax);
		int orgY = randomInt(yMin, yMax);
		Organism next = addOneAt(str, orgY, orgX);
		return next;
	}
	...
	public int getStrainSize(Strain s) {
		LinkedList<Organism> thisStrain = strains.get(s);
		if (thisStrain == null)
			return 0;
		else
	...
	public int getActiveStrainSize(Strain strain) {
		LinkedList<Organism> thisStrain = activeStrains.get(strain);
		if (thisStrain == null)
			return 0;
		else
			return thisStrain.size();
	...
	public int getTombStrainSize(Strain strain) {
		LinkedList<Organism> thisStrain = tombedStrains.get(strain);
		if (thisStrain == null)
			return 0;
		else
			return thisStrain.size();
	...
	public void update() {
		for (Strain s : activeStrains.keySet()) {
			for (Organism o : activeStrains.get(s)) {
				o.update();
			}
		}
		bringOutDead();
	...
	public boolean orgAt(int r, int c) {
		for (List<Organism> orgs : activeStrains.values()) {
			for (Organism o : orgs) {
...
//	}
	...
	public String listLiving() {
		...
		int i = 1;
		int columns = 0;
		int spacePer = this.getLongestLivingOrgName().length() + 15;
		StringBuilder builder = new StringBuilder();
		builder.append("\n" + livingOrgs() + " Living:" + "\n");
		for (Strain strain : activeStrains.keySet()) {
			List<Organism> orgs = activeStrains.get(strain);
			for (Organism o : orgs) {
				String str = o.orgName;
				for (int j = 0; j < this.getLongestLivingOrgName().length()
						- o.orgName.length() + 1; j++) {
					str += " ";
				}
				str += o.distanceFromCenter();
				for (int j = 0; j < 3 - Integer
						.toString(o.distanceFromCenter()).length(); j++) {
					str += " ";
				}
				str += "from OP";
				builder.append(str);
	...
	public String listTombed() {
		for (List<Organism> tomb : tombedStrains.values()) {

			Collections.sort(tomb);
		}
		int i = 1;
		int columns = 0;
		int spacePer = this.getLongestTombedOrgName().length() + 16;
		StringBuilder builder = new StringBuilder();
		builder.append("\n" + tombedOrgs() + " Dead:" + "\n");
		for (Strain strain : tombedStrains.keySet()) {
			List<Organism> tomb = tombedStrains.get(strain);
			for (Organism o : tomb) {
				String str = o.orgName + " of " + o.causeOfDeath;
				builder.append(str);
				columns += str.length();
				while (columns % spacePer != 0) {
					builder.append(" ");
	...
	public String listStrains() {
		...
		for (Strain c : strains.keySet()) {
			LinkedList<Organism> orgsIn = strains.get(c);
			Collections.sort(orgsIn);
			res.append(orgsIn.size() + " in Strain " + c.getStrainName() + ":"
					+ "\n");
			for (Organism o : orgsIn) {
	...
	public String listStrainDataText() {
		StringBuilder res = new StringBuilder();
		for (Strain s : this.strains.keySet())
			res.append("Strain " + s.getStrainName() + ": \t"
					+ this.getActiveStrainSize(s) + "\n");
		return res.toString();
	}

	public String listActiveStrainDataText() {
		StringBuilder res = new StringBuilder();
		for (Strain s : this.activeStrains.keySet())
			res.append("Strain " + s.getStrainName() + ": \t"
					+ this.getActiveStrainSize(s) + "\n");
		return res.toString();
	}
	...
	public String listTombStrainDataText() {
		StringBuilder res = new StringBuilder();
		for (Strain s : this.tombedStrains.keySet())
			res.append("Strain " + s.getStrainName() + ": \t"
					+ this.getTombStrainSize(s) + "\n");
		return res.toString();
	}
	...
	public int tombedOrgs() {
		int i = 0;
		for (List<Organism> orgs : tombedStrains.values()) {
			i += orgs.size();
		}
		return i;
	}
	...
	public int totalOrgs() {
		int i = 0;
		for (List<Organism> orgs : strains.values()) {
			i += orgs.size();
		}
		return i;
	}
	...
	private static String getDateTime() {
		DateFormat dateFormat = new SimpleDateFormat("MM-dd-HHmmss");
		Date date = new Date();
	...
	public void saveImage() {
		...
		File output = new File("./outputImages/" + date + ".png");
		if (!output.exists())
			try {
	...
	public void saveNegative() {
		...
		File outputNeg = new File("./outputImages/" + date + "Neg.png");
		try {
	...
	public void setGreen(int r, int c, int newGreen) {
		Color color = new Color(image.getRGB(r, c));
		Color newColor = new Color(color.getRed(), newGreen, color.getBlue());
		image.setRGB(r, c, newColor.getRGB());
	}

	public void setBlue(int r, int c, int newBlue) {
		Color color = new Color(image.getRGB(r, c));
		Color newColor = new Color(color.getRed(), color.getGreen(), newBlue);
		image.setRGB(r, c, newColor.getRGB());
	}
